...
final class ControlMessageToPartitionStreamCodec
    extends MessageToMessageCodec<ExecutorPartitionMessage.DataTransferControlMessage, PartitionStream> {
  ...
  private void onOutboundPullRequest(final ChannelHandlerContext ctx,
    ...
    checkTransferIdAvailability(
        pullTransferIdToInputStream, ExecutorPartitionMessage.PartitionTransferType.PULL, transferId);
    pullTransferIdToInputStream.put(transferId, in);
    emitControlMessage(ExecutorPartitionMessage.PartitionTransferType.PULL, transferId, in, out);
  ...
  private void onOutboundPushNotification(final ChannelHandlerContext ctx,
    ...
    checkTransferIdAvailability(
        pushTransferIdToOutputStream, ExecutorPartitionMessage.PartitionTransferType.PUSH, transferId);
    pushTransferIdToOutputStream.put(transferId, in);
    in.setTransferIdAndChannel(ExecutorPartitionMessage.PartitionTransferType.PUSH, transferId, ctx.channel());
    emitControlMessage(ExecutorPartitionMessage.PartitionTransferType.PUSH, transferId, in, out);
  ...
  protected void decode(final ChannelHandlerContext ctx,
                        final ExecutorPartitionMessage.DataTransferControlMessage in,
                        final List out) {
    if (in.getType() == ExecutorPartitionMessage.PartitionTransferType.PULL) {
  ...
  private void onInboundPullRequest(final ChannelHandlerContext ctx,
    ...
    outputStream.setTransferIdAndChannel(
        ExecutorPartitionMessage.PartitionTransferType.PULL, transferId, ctx.channel());
  ...
  private void checkTransferIdAvailability(final Map<Short, ?> map,
                                           final ExecutorPartitionMessage.PartitionTransferType transferType,
   ...
   * @param out           the {@link List} into which the created control message is added
   */
  private void emitControlMessage(final ExecutorPartitionMessage.PartitionTransferType transferType,
                                  final short transferId,
                                  final PartitionStream in,
                                  final List out) {
    final ExecutorPartitionMessage.DataTransferControlMessage.Builder controlMessageBuilder
        = ExecutorPartitionMessage.DataTransferControlMessage.newBuilder()
  ...
  private static ExecutorPartitionMessage.PartitionStore convertPartitionStore(
      final Class<? extends PartitionStore> partitionStore) {
    switch (partitionStore.getSimpleName()) {
      case MemoryStore.SIMPLE_NAME:
        return ExecutorPartitionMessage.PartitionStore.MEMORY;
      case SerializedMemoryStore.SIMPLE_NAME:
        return ExecutorPartitionMessage.PartitionStore.SER_MEMORY;
      case LocalFileStore.SIMPLE_NAME:
        return ExecutorPartitionMessage.PartitionStore.LOCAL_FILE;
      case GlusterFileStore.SIMPLE_NAME:
        return ExecutorPartitionMessage.PartitionStore.REMOTE_FILE;
      default:
  ...
  private static Class<? extends PartitionStore> convertPartitionStore(
      final ExecutorPartitionMessage.PartitionStore partitionStoreType) {
