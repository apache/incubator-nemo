    ...
    dag.topologicalDo(irVertex ->
        dag.getIncomingEdgesOf(irVertex).forEach(irEdge -> {
          if (irEdge.getProperty(ExecutionProperty.Key.DataStore) == null) {
            if (irEdge.getProperty(ExecutionProperty.Key.DataCommunicationPattern)
                .equals(DataCommunicationPatternProperty.Value.OneToOne)) {
              irEdge.setProperty(DataStoreProperty.of(DataStoreProperty.Value.MemoryStore));
            } else {
              irEdge.setProperty(DataStoreProperty.of(DataStoreProperty.Value.LocalFileStore));
            }
          }
        }));
    return dag;
