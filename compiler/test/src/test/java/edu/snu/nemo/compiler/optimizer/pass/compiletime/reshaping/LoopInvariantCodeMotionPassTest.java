...
public class LoopInvariantCodeMotionPassTest {
  ...
  public void setUp() throws Exception {
    ...
    final IRVertex vertex4 = groupedDAG.getTopologicalSort().get(7);
    ...
    final List<IREdge> newDAGIncomingEdge = groupedDAG.getIncomingEdgesOf(vertex4);

    alsLoop.getDagIncomingEdges().remove(vertex14);
    alsLoop.getDagIncomingEdges().putIfAbsent(vertex4, new HashSet<>());
    newDAGIncomingEdge.forEach(alsLoop.getDagIncomingEdges().get(vertex4)::add);

    alsLoop.getNonIterativeIncomingEdges().remove(vertex14);
    alsLoop.getNonIterativeIncomingEdges().putIfAbsent(vertex4, new HashSet<>());
    newDAGIncomingEdge.forEach(alsLoop.getNonIterativeIncomingEdges().get(vertex4)::add);

    alsLoop.getBuilder().addVertex(vertex4);
    oldDAGIncomingEdges.forEach(alsLoop.getBuilder()::connectVertices);

    final DAGBuilder<IRVertex, IREdge> builder = new DAGBuilder<>();
    groupedDAG.topologicalDo(v -> {
      if (!v.equals(vertex4) && !v.equals(alsLoop)) {
        builder.addVertex(v);
