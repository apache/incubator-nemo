...
public class PadoCompositePassTest {
  ...
  public void testPadoPass() throws Exception {
    ...
    processedDAG.getIncomingEdgesOf(vertex5).forEach(irEdge -> {
      assertEquals(InterStageDataStoreProperty.Value.MemoryStore, irEdge.getPropertyValue(InterStageDataStoreProperty.class).get());
    ...
    processedDAG.getIncomingEdgesOf(vertex6).forEach(irEdge -> {
      assertEquals(InterStageDataStoreProperty.Value.LocalFileStore, irEdge.getPropertyValue(InterStageDataStoreProperty.class).get());
