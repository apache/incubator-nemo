...
public final class LocalMessageEnvironment implements MessageEnvironment {
  ...
  public <T> void setupListener(
      final ListenerType listenerId, final MessageListener<T> listener) {
    dispatcher.setupListener(currentNodeId, listenerId.name(), listener);
  }

  @Override
  public void removeListener(final ListenerType listenerId) {
    dispatcher.removeListener(currentNodeId, listenerId.name());
  }
